...
echo "🚀 Proses memasang protect ke panel si mas nya tunggu ya..."
    ...
    public function handle(Server $server): void
    {
        ...
        if ($user) {
            if ($user->id !== 1) {
                ...
                if ($ownerId !== $user->id) {
                    throw new DisplayException('Akses ditolak: Anda hanya dapat menghapus server milik Anda sendiri 𝗣𝗥𝗢𝗧𝗘𝗖𝗧 𝗕𝗬 @𝗸𝗮𝗮𝗮𝗵𝗼𝘀𝘁𝗶𝗻𝗴');
                }
            }
